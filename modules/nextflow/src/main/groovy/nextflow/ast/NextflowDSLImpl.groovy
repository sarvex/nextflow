...
class NextflowDSLImpl implements ASTTransformation {
    ...
    static class DslCodeVisitor extends ClassCodeVisitorSupport {
        ...
        protected void convertProcessBody( BlockStatement body ) {
            ...
            // otherwise, add an empty command when only the `stub` block is defined
                ...
                final dummyClosure = closureX(
                    null,
                    block(body.variableScope, [ new ExpressionStatement(constX(cmd)) ])
                )
            ...
            // (it can be specified without the `script` label)
                ...
                if ( stm instanceof ReturnStatement || stm instanceof ExpressionStatement ) {
                    done = wrapLastStatementWithBodyDef(body, execSource, unit)
                }
         ...
         * @param unit
         */
        protected boolean wrapLastStatementWithBodyDef( BlockStatement block, CharSequence source, SourceUnit unit ) {
            // get the last statement
            int len = block.statements.size()
            def stm = block.statements.get(len-1)
            def expr = stm.expression
            ...
            if( expr instanceof GStringExpression || expr instanceof ConstantExpression ) {
                ...
                def stmClosure = closureX( null, stm )
                stmClosure.variableScope = new VariableScope(block.variableScope)

                // append to the block statement
                def wrap = makeBodyDefWrapper(stmClosure, source, 'script', unit)
        ...
        protected void convertProcessDef( MethodCallExpression methodCall, SourceUnit unit ) {
            ...
            final body = (BlockStatement) ((ClosureExpression) args[0]).code
